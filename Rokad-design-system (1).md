# سیستم طراحی رکاد‌اسکول (RokadSchool Design System)
...
رکاد‌اسکول یک پلتفرم آموزشی فارسی‌زبان است. سیستم رنگی آن حول **پنج شخصیت (Persona)** چیده شده که هر کدام یک طیف کامل (Light → Normal → Dark → Darker) به‌همراه حالت‌های تعاملی (`:hover` / `:active`) دارند. این یعنی یک **Design Token** واحد می‌تواند در ۵ تم کاملاً متفاوت بازنمایی شود.
...
در فایل یک افکت سایه سخت (Hard Shadow) با آفست `2.75px 2.75px` و بدون Blur، به رنگ هر تم تعریف شده است — امضای بصری برند رکاد:
